...
const Autobuses = () => {
    ...
    const addOrEdit = (autobus1, resetForm) => {
        if(autobus1.busId == 0){
            console.log(autobus1)
            autobusesService.insertAutobuses(autobus1)
        }
        else{
            console.log(autobus1)
            autobusesService.editAutobuses(autobus1)
        }
        resetForm()
        setOpenPopup(false)
        setRecords(autobus)
    ...
    const DeleteRegister = item => {
        autobusesService.deleteAutobuses(item.id)
                  ...
                  <TableBody>
                      {
                          recordsAfterPagingAndSorting().filter((itemFiltered) =>{
                            ...
                            ? itemFiltered
                            : itemFiltered.tittle.toLowerCase().includes(searchText.toLowerCase())
                          }).map(item =>
