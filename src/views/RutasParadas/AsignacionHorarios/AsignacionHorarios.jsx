...
import AsignacionHorariosForm from './AsignacionHorariosForm';
import {Button, Paper, makeStyles, TableBody, TableRow, TableCell, Toolbar, InputAdornment } from '@material-ui/core';
import * as asignaciónhorariosService from "../../../services/asignaciónhorariosService";
...
const headCells = [
  { id: 'ID', label: 'ID' },
  { id: 'busRouteDescription', label: 'Ruta' },
  { id: 'availableDays', label: 'Días' },
  { id: 'startTime', label: 'Hora inicio' },
  { id: 'finishTime', label: 'Hora fin' },
  { id: 'action', label: 'Acción' },
]
const AsignacionHorario = () => {
    
    // Busqueda de información principal
    const [datalist, setData] = useState([]);
    useEffect(() => {
        axios.get('RouteSchedule/GetRouteSchedule/0')
        .then((response) => {
    ...
    const addOrEdit = (datalist1, resetForm) => {
       console.log(datalist1)
        if(datalist1.id == 0){
          console.log(datalist1)
          asignaciónhorariosService.insertAsignacion(datalist1)
        }
        else{ 
          console.log(datalist1)
          asignaciónhorariosService.editAsignacion(datalist1)
        }
        resetForm()
        setOpenPopup(false)
        setRecords(datalist)
    ...
    const DeleteRegister = item => {
        asignaciónhorariosService.deleteAsignacion(item.id)
        ...
        setFilterFn({
            fn: items => {
                if (target.value == "")
                    ...
                    return items.filter(x => x.busRouteDescription.toLowerCase().includes(target.value))
            }
        })
    }
    ...
    return (
      <React.Fragment>
          <PageHeader
              title="Asignación de horarios a rutas"
              subTitle="OMSAPP"
                  ...
                  <TableBody>
                      {
                          recordsAfterPagingAndSorting().filter((item) =>{
                            ...
                            ? item
                            : item.busRouteDescription.toLowerCase().includes(searchText.toLowerCase())
                          }).map(item =>
                              (<TableRow     key={item.id}>
                                 <TableCell >{item.id}</TableCell>
                                 <TableCell >{item.busRouteDescription}</TableCell>
