...
import * as rutasService from "../../../services/rutasService";
...
const headCells = [
  ...
  { id: 'BRouteInitialStop', label: 'Parada inicial' },
  { id: 'bRouteFinishStop', label: 'Parada final' },
...
const Ruta = () => {
    ...
    useEffect(() => {
        axios.get('BusRoute/Getbusroute/0')
        .then((response) => {
    ...
    const addOrEdit = (datalist1, resetForm) => {
       console.log(datalist1)
        if(datalist1.id == 0){
          console.log(datalist1)
          rutasService.insertRuta(datalist1)
        }
        else{ 
          console.log(datalist1)
          rutasService.editRuta(datalist1)
        }
        resetForm()
        setOpenPopup(false)
        setRecords(datalist)
    ...
    const DeleteRegister = item => {
      rutasService.deleteRuta(item.id)
        ...
        setFilterFn({
            fn: items => {
                if (target.value == "")
                    ...
                    return items.filter(x => x.bRouteDescription.toLowerCase().includes(target.value))
            }
        })
    }
                  ...
                  <TableBody>
                      {
                          recordsAfterPagingAndSorting().filter((item) =>{
                            ...
                            ? item
                            : item.bRouteDescription.toLowerCase().includes(searchText.toLowerCase())
                          }).map(item =>
                              (<TableRow     key={item.id}>
                                 <TableCell >{item.bRouteDescription}</TableCell>
                                 <TableCell >{item.initialStop}</TableCell>
                                  <TableCell>{item.finalStop}</TableCell>
                                  <TableCell>
